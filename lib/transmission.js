    ...
    return new Promise(function(rs, rj) {
        if (data) {
            ...
            for (var i = 0, j = keys.length; i < j; i++) {
                key = keys[i];
                if (!self.methods.session.setTypes[key]) {
            ...
            options = {
                arguments: data,
                method: self.methods.session.set,
                tag: uuid()
            };
            self.callServer(options).then(rs, rj)
        } else {
            options = {
                method: self.methods.session.get,
                tag: uuid()
            };
            self.callServer(options).then(rs, rj)
        }
    ...
    return new Promise(function (rs, rj) {
        ...
        var options = {
            ...
            headers: {
                ...
                'X-Transmission-Session-Id': self.key || '',
        ...
        var res = (self.ssl ? https : http).request(options, onResponse);
